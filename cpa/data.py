    ...
    def __init__(
        ...
        if perturbation_key is not None:
            ...
            if not ("cov_drug_dose_name" in data.obs) or replaced:
                ...
                data.obs["cov_drug_dose_name"] = data.obs["cov_drug_dose_name"].astype('category')
                # NOTE 用knock or not 数据做group rank
                data.obs["cov_drug_dose_name"] = data.obs["knock_gene"].astype('category')

            if not ("rank_genes_groups_cov" in data.uns) or replaced:
                print("Ranking genes for DE genes.")
                # data.obs["knock_gene"] = data.obs["knock_gene"].astype('category')
                # data.obs["split"] = data.obs["split"].astype('category')
                # data.obs["batch"] = data.obs["batch"].astype('category')
                # data.obs["knock_or_not"] = data.obs["knock_or_not"].astype('category')
                rank_genes_groups(data, groupby="cov_drug_dose_name")
                # data.obs["knock_gene"] = data.obs["knock_gene"].astype('category')
                # NOTE
                # rank_genes_groups(data, groupby="knock_or_not")
