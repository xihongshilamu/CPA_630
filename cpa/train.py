...
def evaluate_r2(autoencoder, dataset, genes_control):
    ...
    # print(dataset.pert_categories)
    for pert_category in np.unique(dataset.pert_categories):
        # print(dataset.de_genes)
        # print(pert_category)
        new_pert_category = pert_category.split('_')[1]
        ...
        # )[0]
        de_idx = np.where(
            dataset.var_names.isin(np.array(dataset.de_genes[new_pert_category]))
        )[0]
